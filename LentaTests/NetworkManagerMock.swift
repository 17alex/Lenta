...
final class NetworkManagerMock: NetworkManagerProtocol {
    ...
    let post = Post(id: 0, userId: 0, timeInterval: 2345, description: "Bar", photo: nil, likeUserIds: [0], viewsCount: 1, commentsCount: 0)
    ...
    func register(name: String, login: String, password: String, avatar: UIImage?, complete: @escaping (Result<[User], NetworkServiceError>) -> Void) {
        ...
        registerCallCount += 1
        if name == recivedUserName && login == recivedUserLogin && password == recivedUserPassword && recivadAvatarImage != nil {
            complete(.success([user]))
        } else {
            complete(.failure(networkServiceError))
        }
